...
if ( ! class_exists( 'Sp_Testimonial_Free_Gutenberg_Block_Init' ) ) {
	...
	class Sp_Testimonial_Free_Gutenberg_Block_Init {
		...
		public function sp_testimonial_free_render_shortcode( $attributes ) {
			...
			return $style . '<div id="' . uniqid() . '" class="' . $class_name . '"><a href="' . $edit_page_link . '" target="_blank" class="sp_testimonial_block_edit_button">Edit View</a>' . do_shortcode( '[sp_testimonial id="' . sanitize_text_field( $attributes['shortcode'] ) . '"]' ) . '</div>';
		}
	}
}
